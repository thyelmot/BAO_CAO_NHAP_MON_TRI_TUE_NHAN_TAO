...
\begin{titlepage}
    \begin{tikzpicture}[remember picture, overlay]
        % Khung ngoài
        \draw[line width = 2pt] ($(current page.north west) + (2.2cm,-1.2cm)$) rectangle ($(current page.south east) + (-1.2cm,1.2cm)$);
        % Khung trong
        \draw[line width = 0.5pt] ($(current page.north west) + (2.3cm,-1.3cm)$) rectangle ($(current page.south east) + (-1.3cm,1.3cm)$);
    \end{tikzpicture}
    \centering
    \vspace*{0.5cm}
...
\end{equation}
\textit{Lưu ý: Tại điểm $z=0$, hàm ReLU không có đạo hàm chính thức. Trong thực tế lập trình Machine Learning, ta chọn dưới đạo hàm (subgradient) bằng 0.}

\subsection{Hàm Softmax}
Softmax được sử dụng ở lớp đầu ra ($L$) của bài toán phân loại nhiều lớp, chuyển vector đầu vào thành phân phối xác suất có tổng bằng 1. Nhất quán với quy ước ký hiệu trong Bảng~\ref{tab:symbols}, đầu ra của lớp Softmax tại nơ-ron thứ $c$ được ký hiệu là:
\begin{equation}
...
\textbf{Binary Cross-Entropy Loss:} Dùng cho bài toán phân loại nhị phân ($C=1$, đầu ra Sigmoid):
\begin{equation}
    \mathcal{L} = - \frac{1}{N} \sum_{i=1}^N \left[ y_i \log(\hat{y}_i) + (1 - y_i) \log(1 - \hat{y}_i) \right]
\end{equation}

\textbf{Categorical Cross-Entropy Loss:} Dùng cho bài toán phân loại nhiều lớp ($C > 1$, tính trên $N$ mẫu dữ liệu, mỗi mẫu có $C$ lớp):
\begin{equation}
...
\textbf{Tính Gradient trên Mini-batch $N$ mẫu:} \\
Đạo hàm của hàm mất mát theo đầu vào của lớp cuối cùng (kết hợp Softmax/Sigmoid và Cross-Entropy) được tính bằng:
\begin{equation}
    \delta^{(L)} = A^{(L)} - Y
\end{equation}
Sai số tại lớp ẩn thứ $l$ được lan truyền ngược theo công thức:
\begin{equation}
    \delta^{(l)} = (W^{(l+1)})^T \delta^{(l+1)} \odot f'(Z^{(l)})
\end{equation}
Gradient trung bình của hàm mất mát theo ma trận trọng số và bias trên mini-batch $N$ mẫu:
\begin{align}
    \frac{\partial \mathcal{L}}{\partial W^{(l)}} &= \frac{1}{N} \delta^{(l)} (A^{(l-1)})^T \\
    \frac{\partial \mathcal{L}}{\partial b^{(l)}} &= \frac{1}{N} \sum_{i=1}^N \delta_i^{(l)}
\end{align}
\textit{Lưu ý: Khi hiện thực bằng NumPy theo quy ước ma trận hàng (mỗi hàng là 1 mẫu dữ liệu), phép nhân ma trận được chuyển vị tương ứng: $\delta^{(l)} = \delta^{(l+1)} (W^{(l+1)})^T \odot f'(Z^{(l)})$ và $\frac{\partial \mathcal{L}}{\partial W^{(l)}} = \frac{1}{N} (A^{(l-1)})^T \delta^{(l)}$.}
...
\begin{align}
    ...
    b_t^{(l)} &= b_{t-1}^{(l)} + v_t^{b^{(l)}}
\end{align}
Trong đó, $\beta$ là hệ số Momentum (thường chọn $\beta = 0.9$).
...
\begin{figure}[H]
    ...
    \caption{Quá trình huấn luyện MLP trên Iris với ReLU (Hidden) và Softmax (Output)}
    \label{fig:loss_iris}
    \vspace{-0.2cm}
\end{figure}
...
Trong đề tài này, nhóm đã hoàn thành các mục tiêu đề ra ban đầu, bao gồm:
\begin{itemize}
    ...
    \item \textbf{Nắm vững cơ sở toán học:} Áp dụng chính xác các công thức toán học về Lan truyền thuận (Forward Propagation) và Lan truyền ngược (Backpropagation) sử dụng quy tắc đạo hàm dây chuyền (Chain Rule) trên hệ thống ma trận (Batch Matrix).
...
\begin{thebibliography}{99}
    \bibitem{goodfellow} Ian Goodfellow, Yoshua Bengio, Aaron Courville, \textit{Deep Learning}. MIT Press, 2016.
    \bibitem{bishop} Christopher M. Bishop, \textit{Pattern Recognition and Machine Learning}. Springer, 2006.
    \bibitem{nielsen} Michael A. Nielsen, \textit{Neural Networks and Deep Learning}. Determination Press, 2015.
    \bibitem{rosenblatt} Frank Rosenblatt, ``The Perceptron: A Probabilistic Model for Information Storage and Organization in the Brain,'' \textit{Psychological Review}, vol. 65, no. 6, pp. 386–408, 1958.
    \bibitem{numpy} NumPy Developers, \textit{NumPy Documentation}. Available: \url{https://numpy.org/doc/}
    \bibitem{sklearn} Scikit-learn Developers, \textit{Scikit-learn User Guide}. Available: \url{https://scikit-learn.org/stable/user_guide.html}
    \bibitem{matplotlib} Matplotlib Development Team, \textit{Matplotlib Documentation}. Available: \url{https://matplotlib.org/stable/}
    \bibitem{chollet} François Chollet, \textit{Deep Learning with Python}, 2nd Edition. Manning Publications, 2021.
    \bibitem{duda} Richard O. Duda, Peter E. Hart, David G. Stork, \textit{Pattern Classification}, 2nd Edition. John Wiley \& Sons, 2001.
    \bibitem{iris} Bộ dữ liệu Iris, UCI Machine Learning Repository. Available: \url{https://archive.ics.uci.edu/ml/datasets/iris}
\end{thebibliography}
