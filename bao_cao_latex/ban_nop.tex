...
\begin{equation}
    L' = L + \frac{\lambda}{2N} \sum_{l=1}^L ||W^{(l)}||_F^2
\end{equation}
Gradient và cập nhật trọng số sau khi thêm L2 (chỉ áp dụng cho trọng số, không áp dụng cho bias):
\begin{align}
    \frac{\partial L'}{\partial W^{(l)}} &= \frac{\partial L}{\partial W^{(l)}} + \frac{\lambda}{N} W^{(l)} \\
    W^{(l)} &= W^{(l)} - \eta \left( \frac{\partial L}{\partial W^{(l)}} + \frac{\lambda}{N} W^{(l)} \right)
\end{align}
