...
% --- CẤU HÌNH PHÔNG CHỮ CHO XELATEX ---
\usepackage{fontspec}
\setmainfont{Times New Roman} % Sử dụng font Times New Roman chuẩn Unicode
...
\normalsize
\begin{itemize}
    \item \textbf{Link Google Colab:} \href{https://colab.research.google.com/drive/1K9pYV--SGPx2GZxzJHaitBoXe7rwpSwU?usp=sharing#scrollTo=Vi2DnylCO-6F}{Nhấn vào đây để xem báo cáo (Colab)}
    \item \textbf{Link GitHub:} \href{https://github.com/thyelmot/BAO_CAO_NHAP_MON_TRI_TUE_NHAN_TAO}{Nhấn vào đây để xem mã nguồn (GitHub)}
\end{itemize}
\newpage
...
Giả sử vector đầu vào dạng cột của một mẫu dữ liệu là:
\begin{equation}
    \mathbf{x} = [x_1, x_2, \dots, x_n]^T
\end{equation}
và vector trọng số dạng cột tương ứng là:
\begin{equation}
...
Sigmoid là hàm kích hoạt biến đổi giá trị đầu vào $z$ về khoảng $(0,1)$, thường được sử dụng trong bài toán phân loại nhị phân.
\begin{equation}
...
Softmax được sử dụng ở lớp đầu ra của bài toán phân loại nhiều lớp, chuyển vector đầu ra $z$ thành phân phối xác suất có tổng bằng 1.
\begin{equation}
...
\begin{itemize}
    \item \textbf{Bước 1:} Nhận vector đầu vào $\mathbf{x} = [x_1, x_2, \dots, x_n]^T$
...
\end{align}
\textit{Lưu ý: Các công thức (2.12) và (2.13) được trình bày dưới góc độ toán học cho một mẫu dữ liệu dạng vector cột. Khi triển khai thực tế bằng ma trận trên NumPy cho một batch gồm $N$ mẫu dữ liệu, phép toán sẽ được chuyển vị tương ứng thành $Z = A W + b$.}
...
\textbf{Mean Squared Error (MSE):} Dùng cho bài toán hồi quy.
\begin{equation}
    L = \frac{1}{N} \sum_{i=1}^N (y_i - \hat{y}_i)^2
\end{equation}

\textbf{Cross-Entropy Loss:} Dùng cho bài toán phân loại nhiều lớp trên $N$ mẫu dữ liệu và $C$ lớp.
\begin{equation}
...
\textbf{Tính Gradient:} \\
Sai số tại lớp ẩn $l$ được tính theo:
\begin{equation}
    \delta^{(l)} = (W^{(l+1)})^T \delta^{(l+1)} \odot f'(Z^{(l)})
\end{equation}
Gradient của trọng số và bias tại lớp $l$:
\begin{align}
...
Thuật toán tối ưu (Optimization Algorithm) là phương pháp cập nhật trọng số và bias dựa trên gradient nhằm giảm hàm mất mát.
...
\begin{align}
    W^{(l)} &= W^{(l)} - \eta \frac{\partial L_i}{\partial W^{(l)}} \\
    b^{(l)} &= b^{(l)} - \eta \frac{\partial L_i}{\partial b^{(l)}}
\end{align}

\textbf{SGD with Momentum:}
\begin{align}
    v_t^{(W^{(l)})} &= \beta v_{t-1}^{(W^{(l)})} + \eta \frac{\partial L}{\partial W^{(l)}} \\
    W^{(l)} &= W^{(l)} - v_t^{(W^{(l)})} \\
    v_t^{(b^{(l)})} &= \beta v_{t-1}^{(b^{(l)})} + \eta \frac{\partial L}{\partial b^{(l)}} \\
    b^{(l)} &= b^{(l)} - v_t^{(b^{(l)})}
\end{align}
...
\begin{equation}
    L' = L + \frac{\lambda}{2} \sum_{l=1}^L \|W^{(l)}\|_F^2
\end{equation}
Gradient và cập nhật trọng số tại lớp $l$ sau khi thêm L2:
\begin{align}
    \frac{\partial L'}{\partial W^{(l)}} &= \frac{\partial L}{\partial W^{(l)}} + \lambda W^{(l)} \\
    W^{(l)} &= W^{(l)} - \eta \left( \frac{\partial L}{\partial W^{(l)}} + \lambda W^{(l)} \right)
\end{align}


\chapter{YÊU CẦU CÀI ĐẶT CƠ BẢN}
